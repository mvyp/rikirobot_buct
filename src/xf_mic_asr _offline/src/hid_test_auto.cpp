...
ros::Publisher  no_voice_words_pub;
ros::Publisher recognise_result_pub;
//ros::Publisher voice_text;
...
/*获取离线命令词识别结果*/
std::string Get_Offline_Recognise_Result(int time_per_order,int confidence_z )
{	setlocale(LC_CTYPE, "zh_CN.utf8");
  	setlocale(LC_ALL, "");
	offline_recognise_switch = 1;
	if (offline_recognise_switch == 1) //如果是离线识别模式
	{
		...
		if (result.whether_recognised)
		{
			if (result.whole_content) //确保有反馈
			{
				...
				if (effective_ans.effective_confidence >= confidence_z) //如果大于置信度阈值则进行显示或者其他控制操作
				{
					printf(">>>>>是否识别成功:　[ %s ]\n", "是");
...
/*匹配关键词*/
std::string find_keyword(string text_voice)
{	
	std::string key;
	std::string dataString = text_voice;
    if(dataString.find("客厅") != std::string::npos)
    {
         key = "客厅";
    
    }
    else if(dataString.find("卧室") != std::string::npos) 
    {
         key = "卧室";
        
    }
	 else if(dataString.find("书房") != std::string::npos)
    {
         key = "书房";
        
    }
	else if(dataString.find("走廊") != std::string::npos) 
    {
         key = "走廊";
...
int main(int argc, char *argv[])
{
	...
	ndHandle.param("appid", appid, std::string("0ec1c154"));//appid，需要更换为自己的
	...
	/*　topic 发布识别到的语音*/
	voice_words_pub = n.advertise<std_msgs::String>("goto_command", 1);

	no_voice_words_pub = n.advertise<std_msgs::String>("voiceWords", 1);
	...
	while (ros::ok())
	{
	text_z = Get_Offline_Recognise_Result(7,20);
	string key = find_keyword(text_z);
