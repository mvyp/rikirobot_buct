...
class Follower():
    def __init__(self):
        ...
        self.voice_pub = rospy.Publisher('/voiceWords1', String, queue_size=10)
        self.yolo_pub = rospy.Publisher('/yolo_start', String, queue_size=10)
    ...
    def set_cmd_vel(self, msg):
        ...
        if self.flag_vel:
            self.cmd_vel_pub.publish(self.move_cmd)
            if(self.move_cmd.linear.x>=0.425):
                print('speed high')
            elif(self.move_cmd.linear.x<=0.05):
                tips = "If you want to stop, you have to put your, left hand, on your ,right shoulder"
                self.voice_pub.publish(tips)
                time.sleep(8)
